...
export function CrystalCaseCard({
  ...
  return (
    <article className="group overflow-hidden rounded-[1.5rem] border border-black/10 bg-[#fbfaf7] shadow-sm transition hover:-translate-y-0.5 hover:border-black/20 hover:shadow-[0_20px_70px_rgba(23,23,23,0.14)]">
      <div
      ...
      >
        ...
        <div className="absolute left-3 top-3 flex gap-2">
          <span className="rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-neutral-950 shadow-sm backdrop-blur">
            {product.name}
          </span>
          <span className="hidden rounded-full bg-black/30 px-2.5 py-1 text-xs font-medium text-white backdrop-blur sm:inline-flex">
            {caseItem.styleName}
          </span>
        </div>
      ...
      <div className={featured ? "p-4" : "p-4 sm:p-5"}>
        <div className="mb-3 flex flex-wrap gap-1.5">
          ...
          {caseItem.tags.slice(0, featured ? 1 : 2).map((tag, index) => (
            <span
              ...
              className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-500"
            >
              {tag}
            </span>
          ))}
        ...
        {!featured ? (
          <div className="mt-4 rounded-2xl bg-neutral-100/70 px-3 py-2 text-xs leading-5 text-neutral-600">
